##The code represents a comprehensive financial service with several key components:
...
# calculate_unit_economics(): Breaks down product-level economics
# calculate_equipment_utilization(): Calculates equipment usage
# calculate_financial_projections(): Generates multi-year financial projections
# calculate_key_financial_metrics(): Computes high-level financial metrics


# Projection Capabilities:

# Calculates revenue based on initial values and growth rates
# Handles product-specific revenue calculations
# Accounts for depreciation, interest, and taxes
# Tracks detailed financial metrics


# Scenario Cloning:

# Comprehensive method to clone entire scenarios
# Duplicates scenarios with all associated equipment, products, and cost drivers
...
class FinancialService:
    ...
    def calculate_financial_projections(self, scenario_id, projection_years=5):
        ...
        for year in range(projection_years):
            ...
            # Calculate equipment depreciation and lease costs
            ...
            for equipment in equipment_list:
                ...
                if not equipment.is_leased:
                    # Standard depreciation calculation for purchased equipment
                    ...
                    # Get lease costs
                    from ..services.equipment_service import calculate_lease_costs
                    lease_costs = calculate_lease_costs(equipment.id, year)
                    
                    # Add to financials based on lease type
                    if equipment.lease_type == "standard":
                        # Operating lease - expense only
                        ...
                        # Capital lease - depreciation on books
                        if equipment.lease_type == "fmv_buyout" and lease_costs["book_asset_value"] > 0:
                            # Add implicit asset's depreciation
                            years_to_depreciate = max(5, equipment.lease_term // 12)
                            annual_depreciation = (lease_costs["book_asset_value"] + lease_costs["annual_lease_cost"]) / years_to_depreciate
                            total_depreciation += annual_depreciation
                        
                        # Also add lease costs
                        total_lease_costs += lease_costs["annual_lease_cost"]
                        total_tax_deductible_lease += lease_costs["tax_deductible_amount"]
...
def clone_scenario(session, source_scenario_id, new_name, new_description):
    ...
    for old_product in original.products:
        ...
        session.commit()
        product_map[old_product.id] = new
