...
class GeneralAdministrativeService:
    ...
    def calculate_ga_expenses(self, scenario_id: int, start_year: int, 
        ...
        Returns:
            Dictionary with G&A expense details
        """
        ...
        for year in range(start_year, end_year + 1):
            ...
            if "error" in projection:
                revenue = scenario.initial_revenue * (
                    (1 + scenario.annual_revenue_growth) ** year)
            else:
            ...
            if "error" not in shift_model:
                ...
                for eq_id, eq_data in shift_model.get(
                    "equipment_shift_analysis", {}).items():
                    # Assume average of 1.5 operators per machine per shift
                    manufacturing_headcount += eq_data.get(
                        "required_shifts", 0) * 1.5

            # Get sales headcount
            from .sales_service import SalesService
            sales_service = SalesService(self.session)
            sales_forecast = sales_service.calculate_sales_forecast(
                scenario_id, year, year)

            sales_headcount = 0
            if "error" not in sales_forecast:
                sales_headcount = sales_forecast.get(
                    "staff_requirements", {}).get(year, {}).get(
                    "total_headcount", 0)

            # Calculate G&A expenses and headcount
            ga_expenses["yearly_data"][year] = self._calculate_ga_for_year(
                scenario_id, year, revenue, manufacturing_headcount, 
                sales_headcount)
        ...
        for category in categories:
            ga_expenses["expense_categories"][category] = {
                "total": sum(year_data["categories"].get(category, 0) 
                           for year_data in ga_expenses["yearly_data"].values()),
                "yearly": {year: data["categories"].get(category, 0) 
    ...
    def _calculate_ga_for_year(self, scenario_id: int, year: int, 
                             revenue: float, manufacturing_headcount: float,
                             sales_headcount: float) -> Dict[str, Any]:
        ...
        Args:
            ...
            manufacturing_headcount: Manufacturing headcount
            sales_headcount: Sales headcount
            
        Returns:
            Dictionary with G&A expenses for the year
        """
        ...
        if revenue < 1000000:  # <$1M
            ...
            categories["executive"] = 200000 + (revenue - 1000000) * 0.05
        else:  # >$10M
            categories["executive"] = 650000 + (revenue - 10000000) * 0.02
        ...
        categories["finance"] = max(60000, revenue * 0.03)  # Min $60K or 3% of revenue

        # Legal (scales with revenue)
        categories["legal"] = max(30000, revenue * 0.02)  # Min $30K or 2% of revenue

        # IT (scales with headcount and revenue)
        categories["it"] = 3000 * total_headcount + revenue * 0.01
        ...
        categories["insurance"] = 1000 * total_headcount + revenue * 0.005

        # Professional services (scales with revenue)
        categories["professional_services"] = max(20000, revenue * 0.01)

        # Travel & Entertainment (scales with revenue and headcount)
        categories["travel"] = 2000 * total_headcount + revenue * 0.01
